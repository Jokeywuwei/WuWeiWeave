  ...
  return (
    <section className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-950">{t("dashboard.title")}</h1>
          <p className="mt-1 text-sm text-stone-600">{dashboard?.workspaceRoot ?? t("dashboard.initializing")}</p>
        </div>
        <button
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <MetricTile label={t("dashboard.challenges")} value={dashboard?.challengeCount ?? challenges.length} detail={t("dashboard.trackedTasks")} />
        <MetricTile label={t("dashboard.active")} value={dashboard?.activeChallengeCount ?? 0} detail={t("dashboard.openChallengeStates")} />
        <MetricTile label={t("dashboard.solvers")} value={dashboard?.solverCount ?? solvers.length} detail={t("dashboard.runtimeSessions")} />
        <MetricTile label={t("dashboard.running")} value={dashboard?.runningSolverCount ?? 0} detail={t("dashboard.liveSolverLoops")} />
      </div>

      <div className="grid gap-3 md:grid-cols-4">
        <MetricTile label={t("dashboard.tokens")} value={observability?.providers.totalTokens ?? 0} detail={t("dashboard.providerUsage")} />
        <MetricTile
          label={t("dashboard.cost")}
          value={`$${(observability?.providers.estimatedCostUsd ?? 0).toFixed(4)}`}
          detail={t("dashboard.estimatedSpend")}
        />
        <MetricTile
          label={t("dashboard.slots")}
          value={observability?.scheduler.availableSlots ?? 0}
          detail={`${observability?.scheduler.runningSolvers ?? 0} ${t("dashboard.runningDetail")}`}
        />
        <MetricTile
          label={t("dashboard.events")}
          value={observability?.timeline.length ?? 0}
          detail={t("dashboard.timelineWindow")}
        />
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded border border-stone-200 bg-white">
          <div className="border-b border-stone-200 px-4 py-3 text-sm font-semibold">{t("dashboard.recentChallenges")}</div>
            ...
            {challenges.slice(0, 6).map((challenge) => (
              <div key={challenge.id} className="px-4 py-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs text-stone-500">{translateStatus(challenge.status, t)}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="rounded border border-stone-200 bg-white">
          <div className="border-b border-stone-200 px-4 py-3 text-sm font-semibold">{t("dashboard.recentSolvers")}</div>
          <div className="divide-y divide-stone-100">
            {latestSolvers.length === 0 ? (
              <div className="px-4 py-8 text-sm text-stone-500">{t("dashboard.noSolverSessions")}</div>
            ) : (
              latestSolvers.map((solver) => (
                <div key={solver.id} className="px-4 py-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className="text-xs text-stone-500">{translateStatus(solver.status, t)}</span>
                  </div>
                </div>
              ))
            )}
      ...
      <div className="rounded border border-stone-200 bg-white">
        <div className="border-b border-stone-200 px-4 py-3 text-sm font-semibold">{t("dashboard.eventTimeline")}</div>
